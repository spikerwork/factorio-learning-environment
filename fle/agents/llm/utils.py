...
def remove_whitespace_blocks( messages):
    return [
        ...
        if (isinstance(message['content'], str) and message['content'].strip()) or
    ...
    for message in messages:
        ...
        if 'content_parts' in message:
            for part in message['content_parts']:
                if isinstance(part, dict) and (part.get('type') == 'image' or part.get('type') == 'image_url'):
                    return True
    return False


def format_messages_for_anthropic(messages: List[Dict[str, Any]], system_message: str) -> List[Dict[str, Any]]:
