...
class FactorioGymRegistry:
    ...
    def discover_tasks(self) -> None:
        ...
        if not self._task_definitions_path.exists():
            raise FileNotFoundError(f"Task definitions path not found: {self._task_definitions_path}")
        ...
        for task_file in self._task_definitions_path.glob("*.json"):
            try:
                ...
                goal_description = task_data.get("config", {}).get("goal_description", f"Task: {task_key}")
    ...
    def register_environment(self, 
                           ...
                           model: str = "gpt-4",
                           version: Optional[int] = None,
                           exit_on_task_success: bool = True) -> None:
