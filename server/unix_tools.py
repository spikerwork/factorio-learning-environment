...
def _get_tools_base_path() -> Path:
    ...
    return Path(os.path.dirname(os.path.realpath(__file__))).parent / Path('env') / Path('src') / 'tools'
...
async def ls(path: str = "agent", pattern: str = None) -> str:
    ...
    if pattern:
        ...
        try:
            pattern_path = str(target_path / pattern) if '*' not in pattern else str(target_path) + '/' + pattern
    ...
    files = sorted([entry for entry in entries if entry.is_file()], key=lambda x: x.name)
...
@mcp.tool()
async def find(path: str = "", name_pattern: str = None, content_pattern: str = None, 
         type_filter: str = None, max_depth: int = None) -> str:
    """
...
@mcp.tool()
async def grep(pattern: str, path: str, recursive: bool = True, 
         case_sensitive: bool = False, line_numbers: bool = True) -> str:
    """
...
async def which(command: str) -> str:
    ...
    if matches:
        return f"Tool '{command}' not found directly, but these files might be related:\n- " + "\n- ".join(matches)
    ...
    if admin_dir.exists() and admin_dir.is_dir():
        ...
        if doc_file.exists():
            try:
                ...
                return f"Error reading documentation for admin tool '{command}': {str(e)}"
