...
def make_minimal_observation(**kwargs):
    ...
    return Observation(
        ...
        entities=kwargs.get('entities', ['Entity(name=burner-mining-drill, position=(10,5))']),
        inventory=kwargs.get('inventory', {'iron-ore': 100, 'coal': 50}),
        research=kwargs.get('research', ResearchState(
            technologies={
                'automation': TechnologyState(
                    name='automation', researched=True, enabled=True, level=1,
                    research_unit_count=10, research_unit_energy=30.0,
                    prerequisites=['logistics'], ingredients=[{'item': 'iron-gear-wheel', 'amount': 1}]
                )
            },
        ...
        flows=kwargs.get('flows', ProductionFlows(
            input={'coal': 1.5},
        ...
        task_verification=kwargs.get('task_verification', TaskResponse(success=False, meta={'criteria': 'Place mining drill'})),
        messages=kwargs.get('messages', [AgentMessage(sender='1', content='Need more iron plates', timestamp=1.0)]),
        serialized_functions=kwargs.get('serialized_functions', [{'name': 'dummy_func', 'pickled_function': ''}])
    )
...
def test_inventory_formatting():
    ...
    formatted = formatter.format_inventory([
        {'type': 'iron-ore', 'quantity': 100},
        {'type': 'coal', 'quantity': 50}
    ])
