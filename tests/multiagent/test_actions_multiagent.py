...
from fle.eval.algorithms.independent import TrajectoryRunner
...
from tests.multiagent.test_messages import multi_instance, mock_db_client, mock_config, trajectory_runner
...
""",
    ...
    assert not 'place_furnace_at' in namespace0.persistent_vars
    ...
    assert not 'place_inserter_at' in namespace1.persistent_vars
    ...
    assert furnace_count0 == 2, f"Expected 2 stone furnaces for agent 0, found {furnace_count0}"
    assert furnace_count1 == 2, f"Expected 2 stone furnaces for agent 1, found {furnace_count1}"
