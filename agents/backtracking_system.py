...
class BacktrackingSystem(AgentABC):
    ...
    async def step(self, conversation: Conversation, response: Optional[Response], namespace: FactorioNamespace) -> Policy:
        if self.successful_conv is None: self.successful_conv = copy.deepcopy(conversation)
        if response and response.error and not self.backtracking_agent.memory_full:
            step_output = await self.backtracking_agent.step(self.successful_conv, response, namespace)
            return step_output[0], {"revision_of": response.program_id, "last_successful_program_id": self.last_successful_program_id}
        else:
            # reset the step memory if the response was not an error
            self.backtracking_agent.clear_memory()
            if response and not response.error:
                latest_program_message = Message(role="assistant", content=response.code, metadata={})
                # Add the env message to the conversation
                env_message = Message(role="user", content=response.response, metadata={})
