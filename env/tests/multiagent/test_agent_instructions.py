...
def test_basic_agent_instructions():
    ...
    agent1 = BasicAgent(model="test-model", system_prompt="Base prompt", task=task, agent_idx=0)
    agent2 = BasicAgent(model="test-model", system_prompt="Base prompt", task=task, agent_idx=1)
...
def test_no_agent_instructions():
    ...
    agent = BasicAgent(model="test-model", system_prompt="Base prompt", task=task, agent_idx=0)
...
def test_agent_instructions_index_bounds():
    ...
    with pytest.raises(IndexError):
        agent = BasicAgent(model="test-model", system_prompt="Base prompt", task=task, agent_idx=2)
        assert "### Specific Instructions for Agent" not in agent.system_prompt
