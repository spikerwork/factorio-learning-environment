import pytest
from env.src.entities import Direction, EntityStatus, Position
...
def test_drill_warnings(game):
    ...
    drill = game.place_entity(Prototype.BurnerMiningDrill, position=game.nearest(Resource.IronOre), direction=Direction.UP)
    game.insert_item(Prototype.Coal, drill, 10)
    game.place_entity(Prototype.WoodenChest, position=drill.drop_position, direction=Direction.UP)
    game.sleep(5)
