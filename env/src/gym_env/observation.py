...
class Observation:
    ...
    def from_dict(cls, obs_dict: Dict[str, Any]) -> 'Observation':
        ...
        research = ResearchState(
            technologies={
                tech['name']: TechnologyState(
                    ...
                    ingredients={item['item']: item['amount'] for item in tech['ingredients']}
                )
            ...
            progress={item['name']: item['value'] for item in obs_dict.get('research', {}).get('progress', [])}
        )
        ...
        transformed_flows = {
            'input': {item['type']: item['rate'] for item in flows_dict.get('input', [])},
            'output': {item['type']: item['rate'] for item in flows_dict.get('output', [])},
            'crafted': flows_dict.get('crafted', []),  # Already in correct format
            'harvested': {item['type']: item['amount'] for item in flows_dict.get('harvested', [])},
            'price_list': {item['type']: item['price'] for item in flows_dict.get('price_list', [])} if flows_dict.get('price_list') else None,
            'static_items': {item['type']: item['value'] for item in flows_dict.get('static_items', [])} if flows_dict.get('static_items') else None
        }
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        transformed_flows = {
            ...
            'harvested': [{'type': k, 'amount': v} for k, v in flows_dict['harvested'].items()],
            'price_list': [{'type': k, 'price': v} for k, v in (flows_dict['price_list'] or {}).items()],
            'static_items': [{'type': k, 'value': v} for k, v in (flows_dict['static_items'] or {}).items()]
        }
        ...
        return {
            ...
            'research': {
                ...
                'current_research': self.research.current_research if self.research.current_research is not None else 'None',
                'research_progress': self.research.research_progress,
                'research_queue': self.research.research_queue,
                'progress': [{'name': name, 'value': value} for name, value in self.research.progress.items()]
            },
            'game_info': {
