...
class GymTrajectoryRunner:
    ...
    def _log_trajectory_state(self, iteration_start: float, agent: GymAgent, agent_idx: int, agent_step: int, program: Program, observation: Observation):
    ...
    async def _initialize_trajectory_state(self) -> Tuple[GameState, List[int]]:
        ...
        if self.config.version and self.db_client is not None:
            for agent_idx in range(len(self.agents)):
                current_state, agent_conversation, parent_id, depth = await self.db_client.get_resume_state(
