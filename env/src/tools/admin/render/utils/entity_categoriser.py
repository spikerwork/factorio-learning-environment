...
class EntityCategoriser:
    ...
    def get_entity_category(entity) -> str:
        ...
        if isinstance(entity, Prototype):
            ...
            # Resource types
            if prototype_name in ['iron-ore', 'copper-ore', 'coal', 'stone', 'uranium-ore', 'crude-oil']:
                return prototype_name

            # Check entity class for category
            if entity_class is None:
                # Handle items/resources with no entity class
                if 'ore' in prototype_name or prototype_name == 'coal' or prototype_name == 'stone':
                    return "resource"
                return "item"  # Default for items

            # Check entity class for category
            if issubclass(entity_class, (TransportBelt, Splitter, UndergroundBelt, BeltGroup)):
                return "belt"
            elif issubclass(entity_class, (Inserter, FilterInserter, BurnerInserter)):
                return "inserter"
            elif issubclass(entity_class,
                            (ElectricalProducer, ElectricityPole, Accumulator, Generator, ElectricityGroup)):
                return "power"
            elif issubclass(entity_class, (Pipe, PipeGroup)):
                return "pipe"
            elif issubclass(entity_class, (FluidHandler, MultiFluidHandler, OffshorePump, Pump, StorageTank, Boiler)):
                return "fluid"
            elif issubclass(entity_class, (AssemblingMachine, AdvancedAssemblingMachine, ChemicalPlant, OilRefinery,
                ...
                return "defense"
            elif issubclass(entity_class, (MiningDrill, ElectricMiningDrill, BurnerMiningDrill, PumpJack)):
                return "mining"
        ...
        if isinstance(entity, dict) and 'name' in entity:
            ...
            if resource_name in ['iron-ore', 'copper-ore', 'coal', 'stone', 'uranium-ore', 'crude-oil']:
                return resource_name  # Return the specific resource name as the category
            ...
            if any(belt_type in lower_name for belt_type in ['transport-belt', 'splitter', 'underground-belt']):
                return "belt"
            elif 'inserter' in lower_name:
                return "inserter"
            elif any(power_type in lower_name for power_type in ['electric-pole', 'accumulator', 'engine', 'solar']):
                return "power"
            elif any(fluid_type in lower_name for fluid_type in ['pipe', 'pump', 'tank', 'boiler']):
                return "fluid"
            elif any(prod_type in lower_name for prod_type in ['assembling', 'furnace', 'chemical', 'refinery', 'lab']):
                return "production"
            elif 'chest' in lower_name:
                return "logistics"
            elif any(defense_type in lower_name for defense_type in ['turret', 'wall']):
                return "defense"
            elif any(mining_type in lower_name for mining_type in ['mining-drill', 'pumpjack']):
                return "mining"
        ...
        if isinstance(entity, (TransportBelt, Splitter, UndergroundBelt, BeltGroup)):
            ...
            return "inserter"
        elif isinstance(entity, (ElectricalProducer, ElectricityPole, Accumulator, Generator, ElectricityGroup)):
            return "power"
            ...
            return "fluid"
        elif isinstance(entity, (AssemblingMachine, AdvancedAssemblingMachine, ChemicalPlant, OilRefinery,
                                 Furnace, ElectricFurnace, Lab, RocketSilo)):
            return "production"
            ...
            return "defense"
        elif isinstance(entity, (MiningDrill, ElectricMiningDrill, BurnerMiningDrill, PumpJack)):
            return "mining"
