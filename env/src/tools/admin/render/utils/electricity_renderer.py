...
class ElectricityLayerRenderer(LayerRenderer):
    ...
    def render(self, draw: ImageDraw.ImageDraw,
        ...
        for network in electricity_networks:
            if 'network_id' not in network or 'position' not in network or 'supply_area' not in network:
                continue
            ...
            if (x < min_x - supply_area or x > max_x + supply_area or
                    y < min_y - supply_area or y > max_y + supply_area or
                    supply_area <= 0):
                continue
            ...
            draw.rectangle(
                [img_x - pole_size, img_y - pole_size,
                 img_x + pole_size, img_y + pole_size],
                fill=base_color,
                outline=(0, 0, 0)
            )
