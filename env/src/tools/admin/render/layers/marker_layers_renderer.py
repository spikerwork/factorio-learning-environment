...
class MarkersLayerRenderer(LayerRenderer):
    ...
    def render(self, draw: ImageDraw.ImageDraw,
        ...
        if Layer.ORIGIN in requested_layers and self.config.style["origin_marker_enabled"]:
            if (0 >= boundaries["min_x"] and 0 <= boundaries["max_x"] and
                    0 >= boundaries["min_y"] and 0 <= boundaries["max_y"]):
                origin_x, origin_y = game_to_img_func(0, 0)
                self.shape_renderer.draw_origin_marker(draw, origin_x, origin_y, font)

    def _draw_player_marker(self, draw: ImageDraw.ImageDraw, center_pos: Position,
