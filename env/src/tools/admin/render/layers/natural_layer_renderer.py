...
from env.src.entities import Layer
...
class NaturalLayerRenderer(LayerRenderer):
    ...
    def _draw_trees(self, draw: ImageDraw.ImageDraw, trees: List[Dict],
        ...
        for tree in trees:
            ...
            if 'dead' in tree_name.lower():
                ...
                self._draw_living_tree(draw, img_x, img_y, cell_size, tree_color, tree_name)

        # Reset random seed
        random.seed()

    def _draw_living_tree(self, draw: ImageDraw.ImageDraw, x: float, y: float,
                          cell_size: float, color: tuple, tree_name: str) -> None:
    ...
    def _draw_dead_tree(self, draw: ImageDraw.ImageDraw, x: float, y: float,
                        cell_size: float, color: tuple) -> None:
    ...
    def _draw_rocks(self, draw: ImageDraw.ImageDraw, rocks: List[Dict],
                    game_to_img_func: Callable, boundaries: Dict[str, float]) -> None:
    ...
    def _draw_rock(self, draw: ImageDraw.ImageDraw, x: float, y: float,
                   cell_size: float, color: tuple, rock_name: str) -> None:
