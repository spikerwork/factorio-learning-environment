...
from eval.open.auto_curriculum.recipe_manager import RecipeManager, Recipe
...
class RecipeBasedCurriculum(CurriculumStrategy):
    ...
    def generate_next_task(self, task_history: List[Trace], instance) -> Dict:
        if not task_history or not any([t for t in task_history if t.success]):
            logger.info("No previous tasks - starting with initial copper plate production")
            initial_pl = {
                "input": {"copper-ore": 10},
                "output": {"copper-plate": 10}
            }
            return "Set up initial copper plate production", initial_pl

        # Get accumulated outputs from successful tasks
        accumulated_outputs = self._get_accumulated_outputs(task_history)

        # Ensure copper-ore, stone and iron-ore are in the accumulated outputs
        for prototype in [Prototype.IronOre, Prototype.CopperOre, Prototype.Stone]:
            if prototype.value[0] not in accumulated_outputs.keys():
                accumulated_outputs[prototype.value[0]] = 10
    ...
    def _select_next_recipe(self, craftable_recipes: List[Recipe], task_history: List[Trace]) -> Recipe:
        ...
        def recipe_score(recipe: Recipe) -> float:
            ...
            times_previously_crafted = len(
                [recipe.name in task.achieved_pl["output"]
                for task in task_history if task.success]
            )

            score = (ingredient_count * 0.5 + depth * 0.3) * (1/times_previously_crafted+0.01) + (random()-0.5)
            return score

        choice = max(craftable_recipes, key=recipe_score)
        return choice
